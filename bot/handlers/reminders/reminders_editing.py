...
async def reminders(message: types.Message) -> None:
    ...
    if list_of_rni_reminders:
        ...
        await message.answer(text=reminders, reply_markup=list_to_kb(list_of_rni_reminders, "reminder_rni"))
    else:
        await message.answer("You don't have any reminders without intervals yet.\nType /create_reminder to create one.")

    if list_of_rwi_reminders:
        reminders = "<b>Reminders with intervals:</b>\n"
        reminders += "\n".join([f"<b>{ind+1}.</b> {note[2]}\n" for ind, note in enumerate(list_of_rwi_reminders)])
        await message.answer(text=reminders, reply_markup=list_to_kb(list_of_rwi_reminders, "reminder_rwi"))
    else:
...
async def reminder_rwi_info(callback: types.CallbackQuery):
    ...
    local_time = utc_to_local(callback.from_user.id, True, hour=reminder_data[3], minute=reminder_data[4])
...
async def delete_reminder(callback: types.CallbackQuery):
    ...
    if action != "cancel":
        ...
        if rwi_or_rni == "rwi":
            reminders = "<b>Reminders with intervals:</b>\n"
            reminders += "\n".join([f"<b>{ind+1}.</b> {note[2]}\n" for ind, note in enumerate(list_of_rwi_reminders)])
            await callback.message.edit_text(text=reminders, reply_markup=list_to_kb(list_of_rwi_reminders, "reminder_rwi"))
        else:
            reminders = "<b>Reminders without intervals:</b>\n"
            reminders += "\n".join([f"<b>{ind+1}.</b> {note[2]}\n" for ind, note in enumerate(list_of_rni_reminders)])
            await callback.message.edit_text(text=reminders, reply_markup=list_to_kb(list_of_rni_reminders, "reminder_rni"))
